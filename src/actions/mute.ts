...
export class MuteAction extends SingletonAction<ActionSettings> {
  ...
  override onWillAppear(ev: WillAppearEvent<ActionSettings>): void {
    if (!this.subscribed) {
      ...
      wsManager.on((event) => {
        if (event.type === "connectionChanged" || event.type === "inputMuteChanged") {
          this.updateAllIcons();
        }
      });
    }
  ...
  private getIcon(): string {
    ...
    if (!s.connected || s.inputMuted === null) return icons.micDisconnected;
    return s.inputMuted ? icons.micOff : icons.micOn;
  }
